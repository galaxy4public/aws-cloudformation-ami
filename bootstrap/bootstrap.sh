...
if [ -n "${cfnUseQueue-}" ]; then
	# We are running in a Region where AWS Lambda is not supported,
	# so we will communicate AMI ID through the SNS/SQS

	# we are relying on jq here a lot
	yum -y install jq

	cfnQueueURL=$(aws cloudformation describe-stack-resources \
			--stack-name "$cfnStackId" \
			--logical-resource-id "Queue" \
			| jq -r .StackResources[].PhysicalResourceId \
			| head -1)
	if [ -z "$cfnQueueURL" ]; then
		echo 'ERROR: failed to get the Queue endpoint, stack creation will eventually fail!' >&2
		exit 1
	fi

	# Let's prepare our queue handler first
	cat > /root/queue-handler.sh << __EOF__
#!/bin/bash
export AWS_DEFAULT_REGION="$AWS_DEFAULT_REGION"
export QUEUE_URL="${cfnQueueURL-}"
while : ; do
	...
	MSG_HANDLE=\$(printf '%s' "\$MESSAGE" | jq -r .ReceiptHandle)
	[ -n "\$MSG_HANDLE" ] || exit 1
	aws sqs delete-message --receipt-handle "\$MSG_HANDLE" --queue-url "\$QUEUE_URL"
	MESSAGE=\$(printf '%s' "\$MESSAGE" | jq '.Body|fromjson.Message|fromjson')
	MSG_TYPE=\$(printf '%s' "\$MESSAGE" | jq -r .RequestType | tr '[:lower:]' '[:upper:]')
	RESPONSE_URL=\$(printf '%s' "\$MESSAGE" | jq -r .ResponseURL)
	case "\$MSG_TYPE" in
		CREATE)
			RESPONSE=\$(printf '%s' "\$MESSAGE" \
					| jq '{Status:"SUCCESS",Reason:"Processed",PhysicalResourceId:"Custom",StackId: .StackId, RequestId: .RequestId, LogicalResourceId: .LogicalResourceId, Data: .ResourceProperties.JSON|fromjson}'
			)
			;;
		DELETE|UPDATE)
			RESPONSE=\$(printf '%s' "\$MESSAGE" \
					| jq '{Status:"SUCCESS",Reason:"Ignored",PhysicalResourceId:"Custom",StackId: .StackId, RequestId: .RequestId, LogicalResourceId: .LogicalResourceId}'
			)
			;;
		*)
			RESPONSE=\$(printf '%s' "\$MESSAGE" \
					| jq '{Status:"FAILED",Reason:"Unrecognised request type: '"\$MSG_TYPE"'",PhysicalResourceId:"Custom",StackId: .StackId, RequestId: .RequestId, LogicalResourceId: .LogicalResourceId}'
			)
			;;
	esac
	if [ -n "\$RESPONSE" -a -n "\$RESPONSE_URL" ]; then
		curl -v -X PUT -H 'Content-Type:' --data-binary "\$RESPONSE" "\$RESPONSE_URL"
	fi
	[ "\$MSG_TYPE" == DELETE ] && exit 0 ||:
done
__EOF__
	chmod 0700 /root/queue-handler.sh
	# Execute the queue handler (this will respond on CloudFormation requests for custom resources)
	/root/queue-handler.sh </dev/null >/root/queue-handler.log 2>&1 &
fi
