...
curl -qsSf --retry 900 --retry-delay 1 'https://www.centos.org/keys/RPM-GPG-KEY-CentOS-Official' -o /root/centos.gpg

mkdir -m700 /root/cache
cat > /root/dnf.conf << __EOF__
[main]
gpgcheck=1
installonly_limit=3
best=1
...
if [ "$COMMAND" == 'add' ]; then
	...
	dracut --strip --hardlink --ro-mnt --stdlog 3 --no-hostonly --drivers "xen-blkfront nvme ext4 mbcache jbd2" --force --verbose --show-modules --printsize "/boot/$INITRD" "$KERNEL_VERSION"
	rmdir "$BOOT_DIR_ABS"
	exit 77
fi
__EOF__
...
pkgmanager()
{
	...
	do
		RC=$?
		if [ $I -ge 2 ]; then
			echo "ERROR: dnf failed 3 times with error code '$RC'!" >&2
			exit $RC
		fi
		echo "NOTICE: dnf failed with error code '$RC', re-trying ..." >&2
		I=$(( I + 1 ))
	done
	unset I
}
...
# Download the instance profile from AWS (using IMDSv2)
TOKEN=$(curl -qsS4f --retry 30 --retry-delay 1 -X PUT "http://169.254.169.254/latest/api/token" -H "X-aws-ec2-metadata-token-ttl-seconds: 60")
curl -qsS4f --retry 30 --retry-delay 1 -H "X-aws-ec2-metadata-token: $TOKEN" 'http://169.254.169.254/latest/dynamic/instance-identity/document' -o /root/instance-profile
unset TOKEN
...
do
	I=$((I + 1)
	if [ $I -gt 300 ]; then
		echo "ERROR: waited for 5 minutes for the resolver to be operational, but it is not!" >&2
		exit 1
	fi
...
#!/bin/sh
set -e
[ -z "$SYS_REPO" ] || dnf -y install $SYS_REPO
[ -z "$SYS_PKGS" -a -z "$SYS_UPDATE" ] || dnf -y update
[ -z "$SYS_PKGS" ] || dnf -y install $SYS_PKGS
